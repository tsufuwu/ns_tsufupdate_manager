...
# --- RESOURCE PATH HANDLER (FIXES MISSING ICON/GIF) ---
...
class SwitchToolApp:
    def __init__(self, root):
        self.root = root
        self.lang_code = "VI" # Default language: Vietnamese
    ...
    def setup_window(self):
        # Icon App
        # Use resource_path to locate icon in exe
        try:
    ...
    def show_loading_screen(self):
        ...
        # [IMPORTANT] Run init task first
        threading.Thread(target=self.run_init_tasks, daemon=True).start()

        # [IMPORTANT] Must run animation update loop even if no gif
...
"""
    ...
    def perform_update_download(self, url):
        ...
        try:
            ...
            # Create .bat file to swap executables
...
"""
    ...
    def create_item_card(self, parent, item):
        ...
        # --- AUTO-ADD "DOWNLOAD PAGE" BUTTON ---
        # Logic: Scan urls, if github release is found, deduce the releases page
    ...
    def extract_simple(self, file_path, target_dir, label):
        try:
            ...
            # CASE 2: 7z or RAR
                ...
                # Priority 1: Use System WinRAR/7-Zip
                extracted_ok = self.extract_archive_external(file_path, target_dir)
                
                # Priority 2: Use Python libraries if available
                if not extracted_ok:
    ...
    def helper_extract_any(self, file_path, target_dir):
        ...
        # 1. Try system 7-Zip/WinRAR first
        if self.extract_archive_external(file_path, target_dir):
            return True

        # 2. Try Python libraries
        if f_lower.endswith(".zip"):
            try:
                with zipfile.ZipFile(file_path, 'r') as z: z.extractall(target_dir)
                extracted = True
    ...
    def install_translation_pack(self):
        ...
        # Adjust geometry to prevent text clipping
        ...
        # Use wraplength to handle long text
    ...
    # --- HELPER: System Command Extraction (Optimized for Windows) ---
    def extract_archive_external(self, source_file, dest_dir):
        """Use installed WinRAR or 7-Zip"""
    ...
    def process_translation_task(self, source, root_path, source_type):
        try:
            ...
            # Check ID Strict Regex (Start with 0100, 16 hex chars)
...
if __name__ == "__main__":
    # [IMPORTANT] Set App ID BEFORE creating window
    try:
